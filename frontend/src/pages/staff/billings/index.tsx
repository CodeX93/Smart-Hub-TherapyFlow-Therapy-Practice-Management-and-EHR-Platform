...
const StaffBillings = () => {
  ...
  const { can, apiRoles, canRoute, role } = useStaffAccess();
  const isCustomStaff = isCustomStaffUser(role, apiRoles);
  const canViewBilling = canRoute("billings");
  const canManageBilling = isCustomStaff
    ? can("BILLING_MANAGE") || can("BILLING_EDIT")
    : can("BILLING_MANAGE") || can("BILLING_EDIT");
  ...
  const { data: billingStats, isLoading: isLoadingStats, isError: isStatsError, error: statsError } =
    useGetBillingStatisticsQuery(statisticsArgs, {
      skip: !canViewBilling,
    ...
    <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <BillingTable
          ...
          allowBillingMutations={canManageBilling}
          clientsBasePath="/staff/clients"
        />
      </div>
