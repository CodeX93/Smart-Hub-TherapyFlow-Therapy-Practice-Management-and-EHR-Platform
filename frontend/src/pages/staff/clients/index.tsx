...
import { isCustomStaffUser } from "@/utils/staffPermissions";
...
const StaffClients = () => {
  ...
  const { can, canCapability, apiRoles, role } = useStaffAccess();
  const {
    supervisorStaffMode,
    hideTableScheduledSession,
    hideTableCreateTask,
    hideSessionTabActions,
  } = useStaffPortalRestrictions();
  const isCustomStaff = isCustomStaffUser(role, apiRoles);
  const canCreateClient = can("CLIENT_CREATE");
  const canEditClient = isCustomStaff
    ? can("CLIENT_EDIT")
    : can("CLIENT_EDIT");
  const canDeleteClient = canCapability("deleteClient");
  const canManagePortal = isCustomStaff
    ? can("CLIENT_EDIT")
    : can("CLIENT_EDIT");
            ...
            {isClientsError ? (
              ...
              <>
                <ClientsTable
                  ...
                  hideEdit={!canEditClient}
                  hideDelete={!canDeleteClient}
                  hideScheduledSession={hideTableScheduledSession}
                  hideCreateTask={hideTableCreateTask}
                />
      ...
      {isClientProfileOpen && selectedClient && (
        <div
          className={cn(
            ...
            viewMode === "half" &&
              "fixed top-0 left-112 right-0 bottom-0 z-50 shadow-lg",
            // viewMode === "half" && !isCollapsed &&
            //   "fixed top-0 left-72 right-0 bottom-0 z-50 shadow-lg",
            viewMode === "half" &&
              !isCollapsed &&
              "fixed top-0 left-159 right-0 bottom-0 z-50 shadow-lg",
            viewMode === "full" && "fixed top-0 left-0 right-0 bottom-0 z-50",
        ...
        >
          <ClientProfile
            ...
            onDelete={canDeleteClient ? handleDeleteClient : undefined}
            onCloseFile={canEditClient ? handleCloseClientFile : undefined}
            onActivateFile={
              canEditClient && canReopenFile ? handleActivateClientFile : undefined
            }
            ...
            onUpdatePortalAccess={
              canManagePortal ? handleUpdatePortalAccess : undefined
            }
            onResendPortalActivation={
              canManagePortal ? handleResendPortalActivation : undefined
            }
            requestedAction={requestedProfileAction}
            requestedActionNonce={requestedProfileActionNonce}
            initialTab={profileInitialTab}
            staffMode={supervisorStaffMode}
            schedulingPath="/staff/scheduling"
            hideSessionManageActions={hideSessionTabActions}
            hideTaskManageActions={hideTableCreateTask}
          />
