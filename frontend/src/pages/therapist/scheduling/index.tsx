...
import TherapistDayScheduling from "../../../components/therapist-scheduling-sections/TherapistDayScheduling";
import TherapistWeekScheduling from "../../../components/therapist-scheduling-sections/TherapistWeekScheduling";
import TherapistMonthScheduling from "../../../components/therapist-scheduling-sections/TherapistMonthScheduling";
import TherapistAllSessions from "../../../components/therapist-scheduling-sections/TherapistAllSessions";
...
import { type SchedulingFormValues, type SchedulingSuccessData } from "@/schemas/scheduling.schema";
...
const Scheduling = () => {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      ...
      >
        {selectedType === "Day" && (
          <TherapistDayScheduling
            key={`therapist-day-${refreshKey}`}
            handleEditScheduleClick={handleEditScheduleClick}
          />
        )}
        {selectedType === "Week" && (
          <TherapistWeekScheduling
            key={`therapist-week-${refreshKey}`}
            handleEditScheduleClick={handleEditScheduleClick}
          />
        )}
        {selectedType === "Month" && (
          <TherapistMonthScheduling
            key={`therapist-month-${refreshKey}`}
            handleEditScheduleClick={handleEditScheduleClick}
          />
        )}
        {selectedType === "All" && (
          <TherapistAllSessions
            key={`therapist-all-${refreshKey}-${searchParams.toString()}`}
            handleEditScheduleClick={handleEditScheduleClick}
