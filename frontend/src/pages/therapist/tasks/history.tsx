...
import type { Task, Comment } from "./tasks.static";
import type { TaskFiltersState } from "@/components/therapist/tasks/TaskFilters";
import TaskFilters from "@/components/therapist/tasks/TaskFilters";
...
const TaskHistoryPage = () => {
  ...
  // Stable key for detecting filter changes (excludes page)
  const filterKey = useMemo(
    () => JSON.stringify({ activeTab, filters, loggedInTherapistId, searchQuery }),
    [activeTab, filters, loggedInTherapistId, searchQuery],
  );

  // Ref-based accumulator: resets on filter change, survives re-renders
  ...
  const queryParams = useMemo(
    ...
    [activeTab, filters, loggedInTherapistId, page, searchQuery],
  );
  ...
  const {
    ...
    error: historyError,
    refetch,
  } = useGetTaskHistoryQuery(queryParams, { skip: !loggedInTherapistId });
  ...
  // Derive displayed tasks from accumulator + new page data
    ...
    if (accumulatorRef.current.key !== filterKey) {
      ...
      if (newItems.length > 0) {
        accumulatorRef.current = {
          key: filterKey,
          tasks: [...accumulatorRef.current.tasks, ...newItems],
        };
      }
    }
  ...
  // Open modal first, then fetch — avoids blocking UI
  ...
  const handleUpdateExistingTask = async (updatedTask: Task) => {
    if (!loggedInTherapistId) {
      setToastMessage("Unable to resolve therapist profile.");
      return;
    }
  ...
  return (
    <div className="flex h-[calc(100vh-10.5rem)] min-h-0 flex-col overflow-hidden">
      ...
      {toastMessage ? (
        <div className="fixed right-6 top-6 z-[70] rounded-[0.75rem] border border-[#f3d4d4] bg-[#fff5f5] px-4 py-3 text-sm font-medium text-[#b42318] shadow-[0_12px_24px_rgba(15,23,42,0.10)] flex items-center gap-2">
          <AlertCircle size={16} />
          {toastMessage}
        </div>
      ) : null}
      ...
      <CreateTaskModal
        ...
        onCreate={(taskData) => void handleUpdateTask(taskData)}
      ...
      <EditTaskModal
        ...
        onUpdate={async (taskData) => {
          if (selectedTask) {
            ...
            await handleUpdateExistingTask(updatedTask);
            setEditModalOpen(false);
            setSelectedTask(null);
          }
        }}
