...
import {
  ...
  hasActiveClientInvoiceFilters,
  type ClientInvoiceFilters,
} from "@/utils/clientInvoiceFilters";

interface ClientInvoicesFilterPanelProps {
  filters: ClientInvoiceFilters;
  setFilters: (filters: ClientInvoiceFilters) => void;
}

const ClientInvoicesFilterPanel = ({
  filters,
  setFilters,
}: ClientInvoicesFilterPanelProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [localFilters, setLocalFilters] = useState<ClientInvoiceFilters>(filters);
  ...
  const hasActiveFilters = hasActiveClientInvoiceFilters(filters);
  ...
  const handleClear = () => {
    setLocalFilters(DEFAULT_CLIENT_INVOICE_FILTERS);
        ...
        >
          ...
          <div className="flex flex-1 flex-col gap-6 overflow-y-auto p-4">
            <CustomSelect
              ...
              options={CLIENT_INVOICE_STATUS_OPTIONS}
              isSearch={false}
            />
            ...
            <CustomSelect
              ...
              value={localFilters.insuranceCovered || ""}
              onChange={(value) =>
                setLocalFilters({
                  ...localFilters,
                  insuranceCovered: value || null,
                })
              }
              options={CLIENT_INVOICE_INSURANCE_OPTIONS}
              isSearch={false}
            />
...
export default ClientInvoicesFilterPanel;
