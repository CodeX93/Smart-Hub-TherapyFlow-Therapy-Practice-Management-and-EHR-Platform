...
import type { FormTemplate } from './ClinicalForms';

interface FormAssignmentDropdownProps {
    options: FormTemplate[];
    selectedValues: string[];
    onChange: (values: string[]) => void;
}

const FormAssignmentDropdown = ({ options, selectedValues, onChange }: FormAssignmentDropdownProps) => {
    ...
    return (
        <div className="relative" ref={containerRef}>
            ...
            >
                ...
                >
                    Select form template to assign
                </span>
            ...
            {/* Dropdown Menu */}
            {isOpen && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 max-h-[18.75rem] overflow-y-auto animate-in fade-in zoom-in-95 duration-100 p-2">
                        ...
                        {options.map(option => {
                            ...
                            return (
                                <div
                                    ...
                                    className="flex items-start gap-3 px-3 py-2.5 hover:bg-gray-50 rounded-xl cursor-pointer group transition-colors"
                                >
                                    <div className={cn(
                                        "w-5 h-5 mt-0.5 rounded-md border flex items-center justify-center transition-colors shadow-sm",
                                        isSelected ? "bg-[#1E293B] border-[#1E293B] text-white" : "border-gray-300 group-hover:border-gray-400 bg-white"
                                    )}>
                                        {isSelected && <Check size={12} strokeWidth={3} />}
                                    </div>
                                    <span className={cn("text-sm transition-colors pt-0.5", isSelected ? "text-[#0F172A] font-medium" : "text-[#475569]")}>
                                        {option.title}
                                    </span>
                                </div>
                            );
...
export default FormAssignmentDropdown;
