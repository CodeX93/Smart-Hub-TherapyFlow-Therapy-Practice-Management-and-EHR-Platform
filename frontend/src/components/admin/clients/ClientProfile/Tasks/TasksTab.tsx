...
const ClientTasks = ({
  ...
  const hideTaskManageActions = readOnly || staffMode;
  ...
  } = useClientTaskPages(String(client.id));
  ...
  const toastType = displayedToast?.toLowerCase().includes("success") ? "success" : "error";
  const dismissToast = () => {
    ...
    <div className="flex h-full flex-col space-y-4 px-5 py-5 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {tasks.map((task) => (
              <ClientTaskCard
                key={task.id}
                task={task}
                hideEditDelete={hideTaskManageActions}
                onDelete={(entry) => {
                  setIsTaskDetailsModalOpen(false);
      ...
      <CreateTaskModal
        ...
        client={{
          ...
          assignedTherapistId: client.assignedTherapistId ? String(client.assignedTherapistId) : undefined,
        }}
      />
      ...
      <TaskDetailsModal
        ...
        onDeleteComment={readOnly ? undefined : handleDeleteComment}
        isCommentMutating={isCreatingComment || isUpdatingComment || isDeletingComment}
        readOnly={hideTaskManageActions}
      />
